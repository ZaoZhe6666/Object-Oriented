...
public class Light extends Thread{
	...
	public boolean repOK(){
		...
		try{
			if(random==null || gui==null || light_map==null || light_location==null || map==null){
				return false;
			}
			...
			if(light_change_time>100 || light_change_time<50){
				return false;
			}
			if(last_time<0){
				return false;
	...
	Light(){
		...
		light_change_time=random.get_random(51);
		light_change_time+=49;
	}
			...
			while(shut){
				...
				if(reset){
					for(int i=0;i<light_location.size();i++){
						...
						int way = (map.can_up(line, list)?1:0)+
				...
				if(Driver.time%light_change_time==0){
					clock=!clock;
					for(int i=0;i<light_location.size();i++){
						int line = this.light_location.get(i)/100;
						int list = this.light_location.get(i)%100;
